...
void function(){
    ...
    stack_push= chain_rule(input);
    for(int i=0;i<string.b_no;i++){
        printf("Explain %s\n",stack_push[i]);
    }
...
int find_position_of_start_parenthesis(int position){
    for(int i=string.b_no;i>=0;i--){
        if(position>string.stackstart[i])return i;
    }
}
...
char **chain_rule(char *input){
	...
	for(i=0;i<string.b_no;i++){
        ...
        nearest_parenthesis=find_position_of_start_parenthesis(i);
        for(j=string.stackstart[nearest_parenthesis]; j< string.stackend[i]; j++){
           temp[i][k++]=input[j];
        }
        printf("%s\n", temp[i]);
	}
